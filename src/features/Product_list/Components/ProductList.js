...
import { fetchProductsByFilterAsync, fetchallProductsAsync, selectAllProducts } from "../ProductSlice";
...
// console.log("hello ji");
    ...
    // const uniqueCategoriesSet = new Set(pro.map(product => product.brand));
   
    // const formattedSubCategories = Array.from(uniqueCategoriesSet).map(category => ({
    //   value : category,
    //   label : category,
    //   checked : false
    // }));
    ...
    //   href: "#"
    // }));
    // const formattedCategories = Array.from(uniqueCategoriesSet).map(category => ({
    //   value: category,
  ...
  function handelChangeFilter(e, option, section){
    ...
    let fill = {...filter, [section.id]: option.value};

    setFilter(fill);
    dispatch(fetchProductsByFilterAsync(fill));
    console.log(fill);
  }
